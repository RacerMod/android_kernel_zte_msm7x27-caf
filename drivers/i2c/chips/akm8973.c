...
#include <linux/i2c/akm_sensors_common.h>
#include <linux/i2c/akm8973_private.h>
...
#ifdef AKM_COMPATIBLE
	ret = get_zte_board_id_type();
	if(1 == ret){
		printk(FYA_TAG"This is new hardware version which is used AK8962 in %s\n",__func__);
		printk(FYA_TAG"Do not register AKM8973 compass driver\n");
		return -EAGAIN;
	}
#endif	
